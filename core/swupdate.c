...
static struct option long_options[] = {
	...
	{"no-bootloader-env", no_argument, NULL, 'B'},
...
static void usage(char *programname)
{
	...
	fprintf(stdout,
		...
		" -B, --no-bootloader-env        : disable any bootloader environment modifications\n"
...
#ifdef CONFIG_UBUS
	get_field(LIBCFG_PARSER, elem, "ubus", &sw->ubus_enabled);
#endif
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, main_options,
		...
		case 'm':
			...
			break;
		case 'B':
			swcfg.no_bootloader_env = 1;
			TRACE("bootloader environment changes globally disabled");
			break;
		case 'e':
		...
		case 'P':
			...
			break;
		case 'X':
			strlcpy(swcfg.hookcmd, optarg,
				sizeof(swcfg.hookcmd));
			break;
		default:
